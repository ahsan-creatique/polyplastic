/* ---------------- Child component: ToolingProcessQuantitySection ---------------- */
window.RFQConfig = window.RFQConfig || {};

(function (RFQConfig, LM) {
  "use strict";

  // Process → LOT options, per the Feasibility of Tooling reference sheet.
  var PROCESS_LOT_MAP = {
    "MOLDING": ["MOLD", "CHECKING FIXTURE", "COOLING FIXTURE", "WARPAGE GAUGE"],
    "PLATING": ["PLATING HANGER", "CD GAUGE", "WARPAGE GAUGE", "CHECKING FIXTURE", "MASKING JIG"],
    "PAINTING": ["PAINTING HANGER", "MASKING JIG", "RESTING JIG"],
    "ASSEMBLY": ["SPM", "TAPE CUTTING DIE", "FOAM CUTTING DIE", "RELEASE PAPER CUTTING DIE", "CHECKING FIXTURE", "TAPE PRESSING FIXTURE"]
  };
  ...
  function updateRow(rowId, key, value) {
    var rows = RFQConfig.state.toolingProcessQuantity.rows;
  ...
  function renderTable(rows, components) {
    ...
    return el("div", { class: "sf-table-wrap" }, [
      LM.resizableTable(RFQConfig.state.columnWidths, "toolingProcessQuantity", colgroup, thead, tbody)
    ]);
  }
